...
def test_populate_channels_insertion(db_feed):
    ...
    assert (
        chaine.icon
        == "https://tel.img.pmdstatic.net/fit/https.3A.2F.2Fprd2-tel-epg-img.2Es3-eu-west-1.2Eamazonaws.2Ecom.2Fchannel.2F8107d72a6d5971bf.2Epng/500x500/_/image.png"
    )
    assert (
        chaine.url
        == "https://www.programme-tv.net/programme/chaine/programme-tipik-143.html"
    )
    ...
    assert (
        chaine_last.icon
        == "https://tel.img.pmdstatic.net/fit/https.3A.2F.2Fprd2-tel-epg-img.2Es3-eu-west-1.2Eamazonaws.2Ecom.2Fchannel.2F8d58cd3ce94e9ae8.2Epng/500x500/_/image.png"
    )
    assert (
        chaine_last.url
        == "https://www.programme-tv.net/programme/chaine/programme-gospel-music-tv-346.html"
    )
...
def test_populate_programmes_insertion(db_feed):
    ...
    'url', 'review' fields"""
    prog_9999999 = Programmes.objects.filter(
        date_debut=datetime.datetime(2021, 2, 5, 11, 25, tzinfo=pytz.UTC),
    ...
    prog_60 = Programmes.objects.filter(
        date_debut=datetime.datetime(2021, 2, 5, 11, 59, tzinfo=pytz.UTC),
    ...
    assert (
        prog.icon
        == "https://tel.img.pmdstatic.net/fit/https.3A.2F.2Fprd2-tel-epg-img.2Es3-eu-west-1.2Eamazonaws.2Ecom.2Fprogram.2Ffc2991d58d90eebd.2Ejpg/1280x720/_/image.jpg"
    )
    ...
    assert (
        prog_piege.url
        == "https://www.programme-tv.net/programme/series-tv/r1798-les-4400/854652-pieges/"
    )
