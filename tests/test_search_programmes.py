...
def test_search_programmes_with_a_recherche_field(db_feed):
    ...
    user = User(username="Mel", first_name="Mell", email="mell@gmail.com", password="1234simplepassword")
    user.save()

    recherche = Recherche(recherche="manon des sources",
                        max_resultats=3,
    ...
    prog = Programmes(titre_informatif="Manon des sources",
...
def test_search_programmes_with_letter_accent_and_case_insensitivity(db_feed):
    ...
    recherche = Recherche(recherche='mârCel',
                        max_resultats=3,
                        utilisateur_id=user.id,
                        date_creation=make_aware(datetime.datetime.now())
                        )
